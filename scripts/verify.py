...
def main() -> int:
    ...
    try:
        ...
        hits = classify("SQL injection allows remote code execution via deserialization", ["CWE-89", "CWE-502"])
        ...
        err(f"OSI classifier: {e}"); failures += 1

    try:
        from engine.risk_scoring import RiskInput, score
        r = score(RiskInput(cvss_score=9.8, cwe_ids=["CWE-502"], osi_layers=[6,7], poc_count=2, package_count=12, published="2024-12-01T00:00:00Z"))
        assert 70 <= r.score <= 100, f"expected high score, got {r.score}"
        ok(f"Risk engine returns score={r.score} band={r.band}")
    except Exception as e:
        err(f"Risk scoring: {e}"); failures += 1

    # 2) Ingester imports
    for mod in ("ingest.nvd", "ingest.cwe", "ingest.osv", "ingest.ghsa", "ingest.poc", "ingest.ai_threats"):
        try:
            __import__(mod); ok(f"Imported {mod}")
        except Exception as e:
            err(f"Import {mod}: {e}"); failures += 1
    ...
    try:
        ...
        if n != 7:
            err(f"OSILayer nodes = {n} (expected 7) — run schema first"); failures += 1
        else:
        ...
        err(f"Neo4j: {e} — is `docker compose up -d` running?"); failures += 1
