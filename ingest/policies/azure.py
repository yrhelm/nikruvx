...
def parse_conditional_access(items: list[dict]) -> list[Policy]:
    ...
    for it in items:
        ...
        pol = Policy(
            ...
            scope={
                "state": state,
                "users": (it.get("conditions") or {}).get("users"),
        ...
        if state != "enabled":
            out.append(pol)
            continue  # disabled / report-only - keep visible but no controls
        ...
        if locs.get("includeLocations") and "all" not in [
            str(x).lower() for x in locs.get("includeLocations") or []
        ]:
...
def parse_intune_compliance(items: list[dict]) -> list[Policy]:
    ...
    for it in items:
        ...
        pol.controls.append(
            Control(
                id=cid,
                title=f"Intune compliance: {name}",
                effect="REQUIRE",
                action="device",
                layer=5,
                capability_classes=cap_classes,
                capabilities_mitigated=caps,
                scope={"platform": it.get("@odata.type")},
                raw=it,
            )
        )
        out.append(pol)
    ...
    for nsg in items:
        ...
        for r in rules:
            ...
            if (
                direction == "Inbound"
                and effect == "ALLOW"
                and p.get("sourceAddressPrefix") not in ("*", "Internet")
            ):
            ...
            pol.controls.append(
                Control(
                    ...
                    scope={
                        ...
                        "dst": p.get("destinationAddressPrefix"),
                        "ports": p.get("destinationPortRange"),
                    },
                    raw=p,
                )
            )
