...
def parse_nftables(text: str) -> list[Policy]:
    ...
    for ln, line in enumerate(text.splitlines()):
        s = line.strip()
        if not (("drop" in s) or ("accept" in s) or ("reject" in s)):
            continue
...
def parse_modsecurity(text: str) -> list[Policy]:
    ...
    for ln, line in enumerate(text.splitlines()):
        s = line.strip()
        if not s.startswith("SecRule"):
            continue
        action_match = re.search(r"phase:\d.+?(\bdeny|\ballow|\blog|\bdrop)\b", s)
        effect = (
            "BLOCK"
            if action_match and action_match.group(1) in ("deny", "drop")
            else "MONITOR"
            if "audit" in s.lower() or "log" in s.lower()
            else "ALLOW"
        )
        sl = s.lower()
        cap_classes: list[str] = []
...
def parse_cloudflare(items: list[dict]) -> list[Policy]:
    ...
    for r in items:
        if not isinstance(r, dict):
            continue
        name = (
            r.get("description") or r.get("filter", {}).get("description") or r.get("id", "cf-rule")
        )
        ...
        effect = (
            "BLOCK"
            if action in ("block", "challenge", "js_challenge", "managed_challenge")
            else "ALLOW"
...
def parse_pfsense(text: str) -> list[Policy]:
    ...
    for m in re.finditer(r"<rule>(.*?)</rule>", text, re.S):
        ...
        pol.controls.append(
            Control(
                id=cid,
                title=f"{effect}: {(descr.group(1) if descr else 'rule')}",
                effect=effect,
                action="filter",
                layer=3,
                capability_classes=cap_classes,
