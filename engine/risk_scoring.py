...
def _age_factor(published: datetime | str | None) -> float:
    ...
    age_years = (datetime.now(UTC) - published).days / 365.25
...
def score(inp: RiskInput) -> RiskResult:
    ...
    cvss_part = cvss * 5.0
    if cvss >= 9:
        explanation.append(f"CVSS critical ({cvss})")
    elif cvss >= 7:
        explanation.append(f"CVSS high ({cvss})")
    elif cvss > 0:
        explanation.append(f"CVSS {cvss}")
    ...
    if cwe_ids:
        ...
        worst = max(zip(sevs, cwe_ids, strict=False))
