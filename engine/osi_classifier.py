...
LEXICON: dict[int, list[tuple[str, float]]] = {
    1: [  # Physical
        (r"\b(side[- ]?channel|power analysis|electromagnetic|EMFI|cold[- ]?boot|hardware fault|rowhammer|tempest)\b", 1.0),
    ...
    2: [  # Data Link
        (r"\b(ARP\b|ARP poisoning|ARP spoof|MAC flooding|MAC spoof|VLAN hopping|802\.1[xq]|LLDP|CDP|STP)\b", 1.0),
        (r"\b(switch loop|DHCP starvation|PPP|frame injection|EAP|EAPOL|WPA[2-3]?|wifi handshake|deauth)\b", 0.9),
        (r"\b(layer ?2|data link|ethernet frame)\b", 0.8),
    ],
    3: [  # Network
        (r"\b(IPv?[46]?\s+(spoof|fragment|smurf)|ICMP|routing|BGP|OSPF|EIGRP|RIP\b|NAT|firewall bypass)\b", 1.0),
    ...
    6: [  # Presentation
        (r"\b(TLS\b|SSL\b|HTTPS handshake|certificate (validation|chain|pinning)|cipher suite|heartbleed)\b", 1.0),
        (r"\b(deserialization|serialization|XML (external entity|XXE)|JSON parsing|protobuf|YAML (load|deserial))\b", 1.0),
        (r"\b(unicode (normalization|confusable)|charset|UTF[- ]?[78])\b", 0.85),
        (r"\b(JWT\b|JWS|JWE|JOSE|asn\.?1|x\.?509)\b", 0.9),
        (r"\b(prompt (injection|template|format)|jailbreak|prompt leak)\b", 0.95),  # LLM presentation
        (r"\b(layer ?6|presentation layer|encoding|encryption (flaw|weakness))\b", 0.8),
    ],
    7: [  # Application
        (r"\b(SQL injection|SQLi|XSS|cross[- ]?site scripting|CSRF|SSRF|RCE|remote code execution)\b", 1.0),
        (r"\b(command injection|path traversal|directory traversal|file inclusion|LFI|RFI)\b", 1.0),
        (r"\b(buffer overflow|use[- ]?after[- ]?free|heap (overflow|spray)|integer overflow|format string)\b", 0.95),
        (r"\b(authentication bypass|authorization|privilege escalation|broken access control|IDOR)\b", 0.9),
        (r"\b(API (abuse|key leak|endpoint)|GraphQL|REST endpoint|webhook|OAuth scope)\b", 0.85),
        (r"\b(LLM\b|model (poisoning|extraction|inversion|evasion)|prompt injection|RAG|vector (db|store))\b", 1.0),
