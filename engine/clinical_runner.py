...
from .clinical_ai_corpus import build_corpus, grade
...
def _ask_openai(prompt: str, system: str | None, model: str, api_base: str, api_key: str) -> str:
    msgs: list[dict] = []
    if system:
    ...
    with httpx.Client(timeout=180.0) as c:
        r = c.post(f"{api_base.rstrip('/')}/chat/completions", json=body, headers=headers)
        r.raise_for_status()
        data = r.json()
...
def run_tests(
    ...
    for i, case in enumerate(cases):
        try:
            resp = (
                _ask_openai(case.prompt, case.system_prompt, model, api_base, api_key or "")
                if api_base
                else _ask_ollama(case.prompt, case.system_prompt, model)
            )
...
def summary(model: str | None = None) -> dict:
    ...
    counts = (
        run_read(
            base + " RETURN n.category AS category, n.passed AS passed, count(n) AS n", model=model
        )
        if model
        else run_read(base + " RETURN n.category AS category, n.passed AS passed, count(n) AS n")
    )
    out: dict[str, dict] = {}
    for r in counts:
        cat = r["category"]
    ...
    return {
        ...
        "models": [
            r["m"] for r in run_read("MATCH (n:AIVulnFinding) RETURN DISTINCT n.model AS m")
        ],
    }
...
def main() -> None:
    ...
    p.add_argument(
        "--api-base", default=None, help="Use an OpenAI-compatible /v1 base URL instead of Ollama"
    )
