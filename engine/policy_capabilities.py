...
ALL_CAPS = {
    "RCE", "LOCAL_CODE", "LATERAL_LAN", "INTERNAL_HTTP", "READ_FS", "WRITE_FS",
    "READ_MEM", "AUTH_BYPASS", "PRIV_ESC", "DECRYPT_TLS", "MITM_NET",
    "HW_ACCESS", "MODEL_ACCESS", "DATA_EXFIL", "PHI_DISCLOSURE",
}
...
CONTROL_CLASSES: list[ControlClass] = [
    ...
    ControlClass("waf-rce-rule", {"RCE"}, 7, {"AWS-WAF","Cloudflare","ModSecurity","Azure-FrontDoor"},
    ...
    ControlClass("mfa-required", {"AUTH_BYPASS"}, 5, {"Azure-CA","AWS-IAM","GCP-IAM"},
                 "MFA required for the requested role / scope",
                 "Azure CA: Grant > Require MFA. AWS: aws:MultiFactorAuthPresent in IAM policies."),
    ControlClass("conditional-access", {"AUTH_BYPASS"}, 5, {"Azure-CA"},
                 "Sign-in conditioned on user/device/network/risk", ""),
    ControlClass("device-compliant", {"AUTH_BYPASS","LOCAL_CODE"}, 5, {"Azure-CA","Intune"},
                 "Sign-in requires Intune-compliant device", ""),
    ControlClass("phishing-resistant-auth", {"AUTH_BYPASS"}, 5, {"Azure-CA","Okta","AWS-IAM"},
                 "FIDO2 / passkey / smartcard required (no SMS/TOTP)", ""),
    ControlClass("pim-jit", {"PRIV_ESC"}, 5, {"Azure-CA","AWS-IAM"},
                 "Privileged Identity Management - just-in-time elevation", ""),
    ControlClass("no-standing-admin", {"PRIV_ESC"}, 5, {"Azure-CA","AWS-IAM","GCP-IAM"},
    ...
    ControlClass("imdsv2-required", {"INTERNAL_HTTP"}, 7, {"AWS-IAM"},
                 "EC2 instance metadata v2 enforced (token-required)",
                 "aws ec2 modify-instance-metadata-options --http-tokens required"),
    ControlClass("egress-deny-metadata", {"INTERNAL_HTTP","DATA_EXFIL"}, 3, {"AWS-SG","AWS-NACL","Azure-NSG","GCP-FW"},
                 "Egress firewall blocks 169.254.169.254 / fd00:ec2::254",
                 "iptables -A OUTPUT -d 169.254.169.254 -j REJECT"),
    ControlClass("egress-default-deny", {"DATA_EXFIL","INTERNAL_HTTP"}, 3, {"AWS-SG","Azure-NSG","GCP-FW","iptables","nftables"},
                 "Default-deny outbound, allowlist required destinations",
                 "Pin egress to explicit FQDN/IP set; deny 0.0.0.0/0 by default."),
    ControlClass("egress-proxy", {"DATA_EXFIL","INTERNAL_HTTP"}, 7, {"squid","Cloudflare-Tunnel"},
                 "Forced HTTP egress via inspecting proxy", ""),
    ControlClass("vpc-private-endpoint", {"INTERNAL_HTTP"}, 3, {"AWS-VPC","Azure-PrivateLink","GCP-PSC"},
                 "Internal AWS/Azure/GCP services reachable only via private endpoints", ""),

    # ----- Data exfiltration / DLP -----
    ControlClass("s3-public-block", {"DATA_EXFIL"}, 7, {"AWS-IAM"},
    ...
    ControlClass("kms-resource-policy", {"DATA_EXFIL"}, 6, {"AWS-IAM","GCP-IAM","Azure-KV"},
                 "KMS / KeyVault key policy restricts decryptors", ""),
    ControlClass("dlp-egress", {"DATA_EXFIL"}, 7, {"endpoint","Cloudflare","Symantec"},
                 "Egress DLP scans payloads for sensitive data", ""),

    # ----- TLS / MITM -----
    ControlClass("modern-cipher-only", {"DECRYPT_TLS"}, 6, {"AWS-ALB","nginx","apache","Azure-FrontDoor","Cloudflare"},
                 "TLS 1.2+ minimum, strong cipher suite list",
                 "ssl_protocols TLSv1.2 TLSv1.3; ssl_ciphers ECDHE+AESGCM:CHACHA20;"),
    ControlClass("mtls-required", {"MITM_NET","AUTH_BYPASS"}, 6, {"Istio","Linkerd","AWS-ACM-PCA"},
                 "Mutual TLS for internal service-to-service traffic", ""),
    ControlClass("cert-pinning", {"MITM_NET","DECRYPT_TLS"}, 6, {"app","mobile"},
                 "Certificate / public-key pinning in client", ""),
    ControlClass("hsts-preload", {"DECRYPT_TLS"}, 6, {"nginx","apache","Cloudflare"},
                 "HSTS with preload + includeSubDomains",
                 "Strict-Transport-Security: max-age=63072000; includeSubDomains; preload"),

    # ----- Network segmentation / lateral movement -----
    ControlClass("microsegmentation", {"LATERAL_LAN"}, 3, {"AWS-SG","Azure-NSG","GCP-FW","Calico","Cilium"},
                 "Per-workload firewall rules, not flat network", ""),
    ControlClass("zero-trust-network-access", {"LATERAL_LAN","MITM_NET"}, 3, {"Cloudflare-Access","Tailscale","AWS-VerifiedAccess"},
                 "ZTNA gates instead of VPN", ""),
    ControlClass("trusted-network-only", {"AUTH_BYPASS"}, 5, {"Azure-CA"},
                 "CA policy: sign-in only from named locations / corporate net", ""),

    # ----- Endpoint hardening -----
    ControlClass("disk-encryption", {"READ_FS","DATA_EXFIL"}, 1, {"Intune","Jamf"},
                 "FileVault / BitLocker enforced", ""),
    ControlClass("secure-boot", {"PRIV_ESC","HW_ACCESS"}, 1, {"Intune","Jamf"},
                 "Secure Boot + measured boot enforced", ""),
    ControlClass("attack-surface-reduction", {"RCE","LOCAL_CODE"}, 7, {"Intune","Defender"},
                 "ASR rules block Office macros, executable email attachments, etc.", ""),

    # ----- Memory / kernel -----
    ControlClass("kpti", {"READ_MEM"}, 7, {"kernel"},
                 "Kernel page-table isolation enabled", ""),

    # ----- PHI / HIPAA-specific -----
    ControlClass("phi-encryption-at-rest", {"PHI_DISCLOSURE","READ_FS","DATA_EXFIL"}, 6, {"AWS-IAM","Azure-KV","GCP-IAM","Intune"},
    ...
    ControlClass("phi-encryption-in-transit", {"PHI_DISCLOSURE","DECRYPT_TLS","MITM_NET"}, 6, {"AWS-ALB","nginx","Cloudflare","Azure-FrontDoor"},
                 "PHI in transit over TLS 1.2+ only (HIPAA 164.312(e)(1))",
                 "Min TLS 1.2; HSTS preload; reject older protocols at the ALB / front-door."),
    ControlClass("baa-required-vendors", {"PHI_DISCLOSURE","DATA_EXFIL"}, 7, {"governance"},
    ...
    ControlClass("phi-access-logging", {"PHI_DISCLOSURE"}, 7, {"AWS-IAM","Azure-Monitor","GCP-Logging"},
                 "Audit logs on every PHI access (HIPAA 164.312(b))",
                 "CloudTrail data events on PHI buckets; immutable log retention 6+ years."),
    ControlClass("de-identification", {"PHI_DISCLOSURE"}, 7, {"app"},
    ...
    ControlClass("phi-egress-dlp", {"PHI_DISCLOSURE","DATA_EXFIL"}, 7, {"endpoint","Cloudflare","Symantec"},
                 "Egress DLP scans for PHI patterns (SSN, MRN, DOB+ZIP combos)", ""),
    ControlClass("phi-data-residency", {"PHI_DISCLOSURE"}, 7, {"AWS-IAM","Azure-Policy","GCP-Org"},
    ...
    # ----- AI / ML specific -----
    ControlClass("prompt-input-filter", {"RCE"}, 6, {"app"},
                 "Prompt input scrubber blocking known jailbreak patterns", ""),
    ControlClass("output-filter-llm", {"RCE","DATA_EXFIL"}, 7, {"app"},
                 "LLM output sanitizer before sinks (no shell/SQL/URL execution)", ""),
    ControlClass("model-access-policy", {"MODEL_ACCESS"}, 7, {"AWS-IAM","Azure-AI","GCP-AI"},
